...
namespace IWXMVM::UI
{
        ...
        for (auto it = captureSettings.passes.begin(); it != captureSettings.passes.end();)
        {
            ...
            if (displayPassIndex.has_value() && displayPassIndex.value() == i)
            {
                ...
                if (ImGui::Button(ICON_FA_EYE_SLASH))
                {
                    ...
                    // TODO: do other stuff necessary for the pass type (depth, normal, set clear color to 0 1 0 for greenscreen, etc)
                    // TODO: hide viewmodel if DrawViewmodel is not set, because we cant do that via our R_SetMaterial hook
                }
            } 
    ...
    void CaptureMenu::Render()
    {
        ...
        if (ImGui::Begin("Capture", NULL, flags))
        {
            ...
            {
                if (ImGui::Selectable(magic_enum::enum_name(MenuMode::SinglePass).data(), menuMode == MenuMode::SinglePass))
                {
                    menuMode = MenuMode::SinglePass;
                }

                if (ImGui::Selectable(magic_enum::enum_name(MenuMode::MultiPass).data(), menuMode == MenuMode::MultiPass))
                {
